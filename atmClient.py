...
try:
    ...
    if response_data.get("status") == "success":
        print("Server Authenticated Client.")
        if client_nonce == response_data.get("nonce"):
            print(
                f"Client Authenticated Server from returned nonce: {response_data.get('nonce')}"
            )
        else:
            print(
                f"Client could not auth server due to returned nonce: {response_data.get('nonce')}"
            )
...
while True:
    ...
    packet = json.dumps({"type": "transaction", "data": encrypted_tx, "mac": mac})
